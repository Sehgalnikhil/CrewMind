...
export default function WikiPage() {
  ...
  return (
    <AppShell title="Corporate Wiki">
      <div className="flex flex-col md:flex-row h-full w-full bg-[#05060C] text-white">
        {/* Sidebar */}
        <div className="w-full md:w-72 flex-shrink-0 border-b md:border-b-0 md:border-r border-white/10 bg-[#0B0D14] flex flex-col">
          ...
          <div className="flex overflow-x-auto md:flex-col md:overflow-y-auto p-2 space-x-2 md:space-x-0 md:space-y-1">
            {loadingTopics ? (
              ...
              topics.map((t) => (
                <button
                  ...
                  className={cn(
                    "w-48 md:w-full flex-shrink-0 text-left p-3 rounded-md transition-colors",
                    selectedTopic === t.id 
        ...
        {/* Main Content */}
        <div className="flex-1 min-w-0 overflow-y-auto bg-[#05060C] p-4 sm:p-8 md:p-12">
          {loadingWiki ? (
