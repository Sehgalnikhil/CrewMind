...
export function SimulatorPage() {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      pdf.text(`Scenario Name: ${name || "Baseline Workspace"}`, 20, 30);
      pdf.text(`Generated: ${new Date().toLocaleString()}`, 20, 36);
      
      pdf.setDrawColor(200, 200, 200);
      pdf.line(20, 42, 190, 42);
      
      // 2. Input Parameters (Levers)
      pdf.setFont("helvetica", "bold");
      pdf.setFontSize(16);
      pdf.setTextColor(40, 40, 40);
      pdf.text("1. Input Parameters (Levers)", 20, 55);
      
      pdf.setFontSize(11);
      pdf.setFont("helvetica", "normal");
      pdf.setTextColor(60, 60, 60);
      pdf.text(`• Pricing Change: ${levers.priceChange > 0 ? '+' : ''}${levers.priceChange}%`, 25, 65);
      pdf.text(`• Headcount Delta: ${levers.headcount > 0 ? '+' : ''}${levers.headcount}`, 25, 72);
      pdf.text(`• Marketing Spend: $${levers.marketing}K / mo`, 25, 79);
      pdf.text(`• Churn Assumption: ${levers.churn.toFixed(1)}%`, 25, 86);
      pdf.text(`• EU Market Entry: ${levers.euEntry ? 'Yes' : 'No'}`, 25, 93);
      
      // 3. Projected Business Impact
      pdf.setFont("helvetica", "bold");
      pdf.setFontSize(16);
      pdf.setTextColor(40, 40, 40);
      pdf.text("2. Projected Business Impact", 20, 110);
      
      pdf.setFontSize(11);
      pdf.setFont("helvetica", "normal");
      pdf.setTextColor(60, 60, 60);
      deltas.forEach((d, i) => {
        const val = d.v === Infinity ? "∞" : `${d.label.startsWith("Runway") ? "" : "$"}${d.v}${d.unit === "mo" ? " mo" : d.unit}`;
        pdf.text(`• ${d.label}: ${val}`, 25, 120 + (i * 7));
      });
      
      // 4. Executive Agent Perspectives
      pdf.setFont("helvetica", "bold");
      pdf.setFontSize(16);
      pdf.setTextColor(40, 40, 40);
      pdf.text("3. Executive Agent Perspectives", 20, 155);
      ...
      AGENTS.forEach(a => {
        ...
        // Color code stance
        if (r.stance === "support") pdf.setTextColor(34, 197, 94); // Green
        else if (r.stance === "oppose") pdf.setTextColor(239, 68, 68); // Red
        else pdf.setTextColor(234, 179, 8); // Yellow
        
        pdf.text(`${a.persona.toUpperCase()} (${r.stance.toUpperCase()}) - ${r.confidence}% Confidence`, 25, yPos);
        yPos += 6;
        
        pdf.setFont("helvetica", "normal");
        pdf.setTextColor(60, 60, 60);
        const lines = pdf.splitTextToSize(r.note, 160);
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        pdf.text(`Page ${i} of ${totalPages} - Generated by Crewmind`, 190, 285, { align: "right" });
      }
      
      pdf.save(`Crewmind_Scenario_${name.replace(/\s+/g, '_') || 'Baseline'}.pdf`);
    } catch (e) {
